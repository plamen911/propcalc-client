...
const CoveredRisksForm = ({
    ...
    const calcEarthquakeAmount = (clause1Value, clause2Value, clause3Value) => {
        if (typeof clause1Value === 'undefined' || isNaN(clause1Value)) {
            clause1Value = 0.0;
        }
        if (typeof clause2Value === 'undefined' || isNaN(clause2Value)) {
            clause2Value = 0.0;
        }
        if (typeof clause3Value === 'undefined' || isNaN(clause3Value)) {
            clause3Value = 0.0;
        }

        return parseFloat(clause1Value) + parseFloat(clause2Value) + parseFloat(clause3Value);
    }
    ...
    const handleClauseAmountChange = (clauseId, value) => {
        ...
        setCustomClauseAmounts(prev => {
            ...
            if ((clauseId === 1 || clauseId === 2 || clauseId === getSolarClauseId()) && clauseCheckboxes[6]) {
                ...
                updatedAmounts[6] = calcEarthquakeAmount(clause1Value, clause2Value, clause3Value).toString();
            }
    ...
    const handleClauseCheckboxChange = (clauseId) => {
        ...
        if (clauseId === 6) {
            ...
            if (newCheckedState) {
                let clause6Value = '';
                let clause1Value = customClauseAmounts[1];
                let clause2Value = customClauseAmounts[2];
                let clause3Value = customClauseAmounts[3];
                clause6Value = calcEarthquakeAmount(clause1Value, clause2Value, clause3Value).toString();
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
                <label htmlFor="promotional_code"
                       className="block text-sm sm:text-base font-medium text-white mb-1 sm:mb-2">
                ...
                <div className="mt-1 relative rounded-md shadow-sm">
                    <div className="flex flex-col sm:flex-row gap-2">
                        <div className="flex-grow relative">
                            ...
                            {promoCodeError &&
                                <div className="text-sm sm:text-base text-red-300 mt-1">{promoCodeError}</div>}
                        </div>
                        <div>
                            <button
                    ...
                    {!isCustomPackageExpanded && tariffPresets.filter(preset => preset.active).map((preset) => (
                      ...
                      <span className="text-gray-800 font-medium text-base sm:text-lg mr-1.5 sm:mr-3">
                        {preset.name}
                                            ...
                                            <span
                                                className="text-gray-700 text-sm sm:text-md font-semibold whitespace-nowrap mr-1.5 sm:mr-2">
                                        ...
                                        <div className="flex flex-wrap items-center mt-1.5 sm:mt-1">
                                            <div className="flex items-center">
                                                {+preset.statistics.total_premium > +preset.statistics.total_amount && (
                                                    <span
                                                        className="text-gray-500 text-sm sm:text-md font-semibold whitespace-nowrap line-through mr-1.5 sm:mr-2">
                            {formatCurrency(
                                CalcStatisticsService.calculate(
                                    ...
                                    promoDiscount
                                                ...
                                                <span
                                                    className="text-primary text-sm sm:text-md font-semibold whitespace-nowrap pulse">
                          {formatCurrency(
                              CalcStatisticsService.calculate(
                                  preset.statistics.total_premium,
                                  preset.tax_percent,
                        ...
                        {isCustomPackageExpanded && (
                            <div className="p-3 bg-gray-50">
                                ...
                                <div className="hidden sm:flex justify-between border-b border-gray-300 py-1 mb-1">
                                    ...
                                    <div className="font-medium text-gray-800 text-sm sm:text-base">Застрахователна
                                        сума
                                    </div>
                                </div>
                                ...
                                <div className="space-y-1">
                                    {allClauses.map((clause) => (
                                        ...
                                        (clause.id !== getSolarClauseId() || isSolarByEstateTypeApplicable(formData.estate_type_id)) ? (
                                            <div key={clause.id}
                                                 className="flex flex-col sm:flex-row sm:justify-between sm:items-center border-b border-gray-200 py-2.5 sm:py-2">
                                                <div
                                                    className="flex items-start sm:items-center text-gray-800 text-sm sm:text-base pr-2 flex-1 mb-1.5 sm:mb-0">
                        <span
                            className={`leading-tight ${clause.id === getSolarClauseId() ? 'blink-text text-primary-darker' : ''}`}>
                          {clause.name}
                        </span>
                                                    {clause.description && (
                                                        <InfoModal
                                                            ...
                                                            icon={<EyeIcon
                                                                className="ml-1 text-primary flex-shrink-0 h-5 w-5"/>}
                                                        />
                                                    )}
                                                </div>
                                                    ...
                                                    {clause.allow_custom_amount && (
                                                        <div className="relative rounded-md">
                                                            {(clause.id === 6 || clause.id === 14 || clause.id === 15 || clause.id === 16) && (
                                                                <div
                                                                    className="absolute left-0 top-0 bottom-0 flex items-center pl-3 sm:pl-2 z-10">
                                                            ...
                                                            {isClauseWithCheckbox(clause.id) ? (
                                                                ...
                                                                <>
                                                                    ...
                                                                    <div
                                                                        ...
                                                                        <span
                                                                            className="text-gray-700 text-sm sm:text-base font-medium">{currencySymbol}</span>
                                                                    </div>
                                                                </>
                                                            ) : (
                                                                ...
                                                                <div className="p-0">
                                                                    ...
                                                                    <div className="text-center"
                                                                        ...
                                                                        <Typography
                                                                            ...
                                                                            fontWeight="medium" sx={{lineHeight: 1.1}}>
                                ...
                                <div
                                    className="mt-4 sm:mt-6 rounded-lg overflow-hidden border border-gray-300 shadow-md">
                                    <div className="border-b border-gray-200 bg-gray-100 p-2.5 sm:p-3">
                                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
                                            <div className="flex items-start sm:items-center mb-1 sm:mb-0">
                                                <span
                                                    className="inline-block w-2 h-2 bg-blue-500 rounded-full mr-2 mt-1 sm:mt-0 flex-shrink-0"></span>
                                                <span
                                                    className="uppercase text-gray-800 text-xs sm:text-sm font-medium leading-tight">
                          Застрахователна премия
                        </span>
                                            </div>
                                            <div
                                                className="text-primary font-semibold text-sm sm:text-base sm:ml-2 self-end sm:self-auto">
                                                {formatCurrency(
                                                    CalcStatisticsService.calculate(
                                    ...
                                    <div className="border-b border-gray-200 bg-gray-100 p-2.5 sm:p-3">
                                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
                                            <div className="flex items-start sm:items-center mb-1 sm:mb-0">
                                                <span
                                                    className="inline-block w-2 h-2 bg-green-500 rounded-full mr-2 mt-1 sm:mt-0 flex-shrink-0"></span>
                                                <span
                                                    className="uppercase text-gray-800 text-xs sm:text-sm font-medium leading-tight">
                                    ...
                                    {promoCodeValid && promoDiscount && customPackageStatistics.statistics.discounted_premium && (
                                        <div className="border-b border-gray-200 bg-gray-100 p-3">
                                            <div className="flex justify-between items-center">
                                                <div className="flex items-center">
                                                    <span
                                                        className="inline-block w-2 h-2 bg-green-500 rounded-full mr-2"></span>
                                                    <span
                                                        className="uppercase text-gray-800 text-xs sm:text-sm font-medium">
                              Застрахователна премия след приложен промо код {promoDiscount}%
                            </span>
                                                </div>
                                                <div
                                                    className="font-semibold text-base sm:text-lg ml-2 whitespace-nowrap">
                                  ...
                                  CalcStatisticsService.calculate(
                                    ...
                                    <div className="border-b border-gray-200 bg-gray-100 p-2.5 sm:p-3">
                                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
                                            <div className="flex items-start sm:items-center mb-1 sm:mb-0">
                                                <span
                                                    className="inline-block w-2 h-2 bg-yellow-500 rounded-full mr-2 mt-1 sm:mt-0 flex-shrink-0"></span>
                                                <span
                                                    className="uppercase text-gray-800 text-xs sm:text-sm font-medium leading-tight">
                                    ...
                                    <div className="border-b border-gray-200 bg-gray-100 p-2.5 sm:p-3">
                                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
                                            <div className="flex items-start sm:items-center mb-1.5 sm:mb-0">
                                                <span
                                                    className="inline-block w-3 h-3 bg-red-500 rounded-full mr-2 mt-0.5 sm:mt-0 flex-shrink-0"></span>
                                                <span
                                                    className="uppercase text-gray-800 text-xs sm:text-sm font-medium leading-tight">
                          Общо дължима сума за една година
                                            ...
                                            <div
                                                className="text-primary font-semibold text-sm sm:text-base sm:ml-2 self-end sm:self-auto pulse">
                                                {formatCurrency(
                                                    CalcStatisticsService.calculate(
                                                        customPackageStatistics.statistics.total_premium,
                                                        customPackageStatistics.tax_percent,
                                ...
                                <div className="flex flex-col w-full sm:items-center mt-5">
                                    {validationError && (
                                        <div
                                            className="mb-3 text-red-600 text-sm text-center w-full bg-red-100 p-2 rounded-md border border-red-300">
                                            {validationError}
                                        </div>
                                    )}
                                    <ProceedButton
